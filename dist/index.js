...
`));let C=te.replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(/INPUT\-FIELDS/g,c.join(`
...
`);}),F.push(s)):s.includes(`const initialState: ${e}Type = {};`)?(F.push(`const initialState: ${e}Type = {`),F.push(n)):F.push(s);}),$.writeFileSync(A,F.join(`
`));let Ee=ne.replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(/INPUT\-FIELDS/g,c.join(`
...
`);}),L.push(s)):s.includes(`const initialState: ${e}Type = {};`)?(L.push(`const initialState: ${e}Type = {`),L.push(n)):L.push(s);}),$.writeFileSync(z,L.join(`
`));let ve=$.readFileSync(J),ue=JSON.parse(ve.toString());ue[0].items.push({label:e,to:`/${e.toLowerCase()}`}),$.writeFileSync(J,JSON.stringify(ue,null,2)),V.start(`Creating types/${e.toLowerCase()}.d.ts`);let Se=re.replace(/XXXXX/g,e).split(`
`)[0]+`
...
`)).replace(a.entity,n.join(`
`)).replace(a.zod,o.join(`
`)).replace(a.uniqueFields,r.join(", ")).replace(a.checkExistingCreateEntity,q(e,r)).replace(a.checkExistingUpdateEntity,_(e,r)),A=$.readFileSync(`${m}/server/XXXXXEntity.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(a.interface,i.join(`
`)).replace(a.schema,c.join(`
`)).replace(a.entity,n.join(`
`)).replace(a.zod,o.join(`
`)).replace(a.uniqueFields,r.join(", ")).replace(a.checkExistingCreateEntity,q(e,r)).replace(a.checkExistingUpdateEntity,_(e,r)),z=`${y}/${e}Controller.ts`,J=`${y}/${e}Router.ts`,ee=`${y}/${e}.dto.ts`,te=`./src/Database/Entities/${e}Entity.ts`;$.writeFileSync(z,X),$.writeFileSync(J,d),$.writeFileSync(ee,v),$.writeFileSync(te,A);let ne=$.readFileSync("./src/Database/CollectionNames.ts").toString().split(`
`).map(p=>p.includes("export const ")?p+`
${e}Collection: "${t.collectionName}",`:p).join(`
`);$.writeFileSync("./src/Database/CollectionNames.ts",ne);let re=$.readFileSync("./src/Microservices/ApiRouter.ts").toString().split(`
...
`);$.writeFileSync(i,o),$.writeFileSync(c,y),Q.succeed(`Removed CRUD: ${t}`);}));})}function pe(){$.removeSync("./.git"),$.removeSync("./yarn.lock"),$.removeSync("./src/Microservices/XXXXX"),$.removeSync("./src/Database/Entities/XXXXXEntity.ts");}function Z(e){return u(this,null,function*(){return new Promise((t,r)=>{exec(e,()=>{t(null);});})})}var N=Ce({color:"blue",indent:2});function le(e){return u(this,null,function*(){let t=e.toLowerCase();N.start("Scaffolding project..."),$.ensureDirSync(t),process.chdir(t);let[,,r,n]=yield Promise.all([we().clone("https://github.com/kuvamdazeus/admin-starter-react","webapp"),we().clone("https://github.com/kuvamdazeus/node-starter-kit","server"),Fe("https://gist.githubusercontent.com/kuvamdazeus/89117514d4ef61f9a09e1cd9bf0cba4f/raw/639870454bf9b2626ae81c663e91bc422f0f9932/kit.config.json").then(i=>i.text()),Fe("https://gist.githubusercontent.com/kuvamdazeus/08e407c3188c08c0d29012f85dd3c9d9/raw/f4d6b2e429063bf454e5d2d805f3a7806b56d491/node-starter-kit-env.txt").then(i=>i.text())]);$.writeFileSync("./kit.config.json",r),$.ensureDirSync(`${m}`),$.ensureDirSync(`${m}/webapp`),$.ensureDirSync(`${m}/server`),O("webapp",()=>{var i;$.copyFileSync("./src/screens/XXXXX/XXXXX.tsx",`${m}/webapp/XXXXX.tsx`),$.copyFileSync("./src/screens/XXXXX/CreateXXXXX.tsx",`${m}/webapp/CreateXXXXX.tsx`),$.copyFileSync("./src/screens/XXXXX/EditXXXXX.tsx",`${m}/webapp/EditXXXXX.tsx`),$.copyFileSync("./src/types/xxxxx.d.ts",`${m}/webapp/xxxxx.d.ts`),$.writeFileSync("./.env",`VITE_BASE_URL = "${(i=U())==null?void 0:i.backendUrl}"`),M();}),O("server",()=>{$.copyFileSync("./src/Microservices/XXXXX/XXXXXRouter.ts",`${m}/server/XXXXXRouter.ts`),$.copyFileSync("./src/Microservices/XXXXX/XXXXXController.ts",`${m}/server/XXXXXController.ts`),$.copyFileSync("./src/Microservices/XXXXX/XXXXX.dto.ts",`${m}/server/XXXXX.dto.ts`),$.copyFileSync("./src/Database/Entities/XXXXXEntity.ts",`${m}/server/XXXXXEntity.ts`),$.writeFileSync("./.env",n),pe();}),N.succeed(`Created "${t}" successfully!`),N.start("Installing dependencies..."),yield S("webapp",()=>u(this,null,function*(){yield Z("yarn install");})),yield S("server",()=>u(this,null,function*(){yield Z("yarn install");})),N.succeed("Installed dependencies successfully!");})}var m=Ke.homedir()+"/.adminkit",K=new Command;K.name("admin-starter").description("CLI to setup & manager admin UIs").version("0.1.0");K.command("scaffold").description("Scaffold a new admin project by using templates").argument("<projectName>","Name of the project").action(le);K.command("addscreen").description("Add a new screen").argument("<screenName>","Name of the screen").action(B);K.command("removescreen").description("Removes an existing screen").argument("<screenName>","Name of the screen").action(ae);K.command("sync").description('Add screens defined in "kit.config.json" to the project').action(ce);K.parse();
