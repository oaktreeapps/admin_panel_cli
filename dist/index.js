...
var u=(e,t,r)=>new Promise((n,i)=>{var a=X=>{try{y(r.next(X));}catch(d){i(d);}},o=X=>{try{y(r.throw(X));}catch(d){i(d);}},y=X=>X.done?n(X.value):Promise.resolve(X.value).then(a,o);y((r=r.apply(e,t)).next());});function M(){let e=['import XXXXXPage from "./screens/XXXXX/XXXXX"','import EditXXXXXPage from "./screens/XXXXX/EditXXXXX"','import CreateXXXXXPage from "./screens/XXXXX/CreateXXXXX"','<Route path="xxxxx" element={<XXXXXPage />} />','<Route path="xxxxx/create" element={<CreateXXXXXPage />} />','<Route path="xxxxx/edit/:id" element={<EditXXXXXPage />} />'];$.removeSync("./.git"),$.removeSync("./src/screens/XXXXX"),$.removeSync("./src/types/xxxxx.d.ts");let n=$.readFileSync("./src/main.tsx").toString().split(`
`).filter(i=>e.filter(a=>i.includes(a)).length===0);$.writeFileSync("./src/main.tsx",n.join(`
...
></Column>`;var V=Ce({color:"blue",indent:2});function re(e,t){return u(this,null,function*(){let r="",n="",i=[],a=[],o=[],y=[],X=["InputSwitch"];t.crudFields.forEach((s,I)=>{let w="",P="";switch(s.required&&!X.includes(s.type)&&i.push(s.name),s.type){case"InputText":s.tableDisplay&&o.push(R(s.name)),a.push(Xe(s)),w="string",P='""';break;case"InputTextarea":s.tableDisplay&&o.push(R(s.name)),a.push(ye(s)),w="string",P='""';break;case"InputNumber":s.tableDisplay&&o.push(R(s.name)),a.push(me(s)),w="number",P="0";break;case"Dropdown":s.tableDisplay&&o.push(R(s.name)),a.push(de(s)),y.push({fieldName:s.name,options:s.options||[]}),w="string",P='""';break;case"RadioButton":s.tableDisplay&&o.push(R(s.name)),a.push($e(s,s.options||[])),w="string",P='""';break;case"InputSwitch":s.tableDisplay&&o.push(R(s.name)),a.push(ge(s)),w="boolean",P="false";break}I===0&&(r+=`  id?: string;
`,n+=`  id: "",
`),r+=`  ${s.name}: ${w};
`,n+=`  ${s.name}: ${P},
...
`));let C=ee.replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(/INPUT\-FIELDS/g,a.join(`
...
`));let Ee=te.replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(/INPUT\-FIELDS/g,a.join(`
...
`),Ie=[`      <Route path="${e.toLowerCase()}" element={<${e}Page />} />`,`      <Route path="${e.toLowerCase()}/create" element={<Create${e}Page />} />`,`      <Route path="${e.toLowerCase()}/edit/:id" element={<Edit${e}Page />} />`],Te=[`import ${e}Page from "./screens/${e}/${e}"`,`import Edit${e}Page from "./screens/${e}/Edit${e}"`,`import Create${e}Page from "./screens/${e}/Create${e}"`],H=[];H.push(...Te),be.forEach(s=>{s.includes("</Route>")&&Ie.forEach(I=>{H.push(I);}),H.push(s);}),$.writeFileSync("./src/main.tsx",H.join(`
...
`;}),r};var c={interface:"/*INTERFACE_FIELDS*/",schema:"/*SCHEMA_FIELDS*/",entity:"/*ENTITY_FIELDS*/",zod:"/*ZOD_FIELDS*/",uniqueFields:"/*UNIQUE_FIELDS*/",checkExistingUpdateEntity:"/*CHECK_EXISTING_UPDATE_ENTITY*/",checkExistingCreateEntity:"/*CHECK_EXISTING_CREATE_ENTITY*/"};function se(e,t){return u(this,null,function*(){let r=t.crudFields.filter(p=>p.unique).map(p=>p.name),n=[],i=[],a=[],o=[];t.crudFields.forEach(({name:p,type:g,required:C,unique:F})=>{n.push(`${p}: entity.${p},`),g==="InputText"||g==="InputTextarea"||g==="Dropdown"||g==="RadioButton"?(i.push(`${p}${C?"":"?"}: string;`),a.push(`${p}: { type: String, required: ${C}, unique: ${F} },`),o.push(`${p}: z.string()${C?".nonempty()":".optional()"},`)):g==="InputNumber"?(i.push(`${p}${C?"":"?"}: number;`),a.push(`${p}: { type: Number, required: ${C}, unique: ${F} },`),o.push(`${p}: z.number()${C?"":".optional()"},`)):g==="InputSwitch"&&(i.push(`${p}${C?"":"?"}: boolean;`),a.push(`${p}: { type: Boolean, required: ${C}, unique: ${F} },`),o.push(`${p}: z.boolean()${C?"":".optional()"},`));});let y=`./src/Microservices/${e}`,X=$.readFileSync(`${m}/server/XXXXXController.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(c.interface,i.join(`
`)).replace(c.schema,a.join(`
`)).replace(c.entity,n.join(`
`)).replace(c.zod,o.join(`
`)).replace(c.uniqueFields,r.join(", ")).replace(c.checkExistingCreateEntity,q(e,r)).replace(c.checkExistingUpdateEntity,_(e,r)),d=$.readFileSync(`${m}/server/XXXXXRouter.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(c.interface,i.join(`
`)).replace(c.schema,a.join(`
`)).replace(c.entity,n.join(`
`)).replace(c.zod,o.join(`
`)).replace(c.uniqueFields,r.join(", ")).replace(c.checkExistingCreateEntity,q(e,r)).replace(c.checkExistingUpdateEntity,_(e,r)),v=$.readFileSync(`${m}/server/XXXXX.dto.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(c.interface,i.join(`
`)).replace(c.schema,a.join(`
`)).replace(c.entity,n.join(`
`)).replace(c.zod,o.join(`
`)).replace(c.uniqueFields,r.join(", ")).replace(c.checkExistingCreateEntity,q(e,r)).replace(c.checkExistingUpdateEntity,_(e,r)),K=$.readFileSync(`${m}/server/XXXXXEntity.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(c.interface,i.join(`
`)).replace(c.schema,a.join(`
`)).replace(c.entity,n.join(`
`)).replace(c.zod,o.join(`
`)).replace(c.uniqueFields,r.join(", ")).replace(c.checkExistingCreateEntity,q(e,r)).replace(c.checkExistingUpdateEntity,_(e,r)),N=`${y}/${e}Controller.ts`,J=`${y}/${e}Router.ts`,z=`${y}/${e}.dto.ts`,ee=`./src/Database/Entities/${e}Entity.ts`;$.writeFileSync(N,X),$.writeFileSync(J,d),$.writeFileSync(z,v),$.writeFileSync(ee,K);let te=$.readFileSync("./src/Database/CollectionNames.ts").toString().split(`
