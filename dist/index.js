...
`));}var W=(e=process.cwd())=>h.readdirSync(e).includes("kitconfig"),b=(e,o)=>a(void 0,null,function*(){let r=process.cwd().split("/").at(-1),t;return r===e||W()&&e==="root"?t=yield o():W()?(process.chdir(e),t=yield o(),process.chdir("..")):e==="root"?(process.chdir(".."),t=yield o(),process.chdir(r)):(process.chdir(".."),process.chdir(e),t=yield o(),process.chdir(".."),process.chdir(r)),t}),D=(e,o)=>{let r=process.cwd().split("/").at(-1),t;return r===e||W()&&e==="root"?t=o():W()?(process.chdir(e),t=o(),process.chdir("..")):e==="root"?(process.chdir(".."),t=o(),process.chdir(r)):(process.chdir(".."),process.chdir(e),t=o(),process.chdir(".."),process.chdir(r)),t},G=()=>D("root",()=>{let e,o=h.readdirSync(".");return o.includes("webapp")&&o.includes("server")?e="both":o.includes("webapp")?e="webapp":o.includes("server")?e="server":e="INVALID_STATE",e}),j=()=>D("root",()=>process.cwd()+"/.template");function ce(){h.removeSync("./.git"),h.removeSync("./src/Microservices/XXXXX"),h.removeSync("./src/Database/Entities/XXXXXEntity.ts");}function L(e){return a(this,null,function*(){return new Promise(o=>{exec(e,()=>{o(null);});})})}var he=()=>`REST_API_PORT=3005
...
></Column>`,je=e=>`<Column header="${T(e)}" body={(rowData) => imageBodyTemplate(rowData, "${e}")}></Column>`;var P={tableColumns:"{/*TABLE_COLUMNS*/}",initialState:"/*INITIAL_STATE_FIELDS*/",interface:"/*INTERFACE_FIELDS*/",input:"{/*INPUT_FIELDS*/}",validate:"/*VALIDATE_FIELDS*/"};function ue(e,o){return a(this,null,function*(){let r=j(),t="",s="",p=[],c=[],i=[],d=[],y=["InputSwitch"];o.crudFields.forEach((n,A)=>{let f=n.widget||n.datatype,x="",F="";n.required&&!y.includes(n.widget||n.datatype||"")&&p.push(n.name),f==="InputText"||f==="String"?(n.tableDisplay&&i.push(I(n.name)),c.push(be(n)),x="string",F='""'):f==="InputTextarea"?(n.tableDisplay&&i.push(I(n.name)),c.push(Ie(n)),x="string",F='""'):f==="InputNumber"||f==="Number"?(n.tableDisplay&&i.push(I(n.name)),c.push(Ee(n)),x="number",F="0"):f==="Dropdown"?(n.tableDisplay&&i.push(I(n.name)),c.push(Te(n)),d.push({fieldName:n.name,options:n.options}),x="string",F='""'):f==="RadioButton"?(n.tableDisplay&&i.push(I(n.name)),c.push(ve(n)),d.push({fieldName:n.name,options:n.options}),x="string",F='""'):f==="ImageFileUpload"?(n.tableDisplay&&i.push(je(n.name)),c.push(_e(n,o.name.toLowerCase())),x="string",F='""'):f==="InputSwitch"||f==="Boolean"?(n.tableDisplay&&i.push(I(n.name)),c.push(Pe(n)),x="boolean",F="false"):f==="Calendar"?(n.tableDisplay&&i.push(I(n.name)),c.push(Re(n)),x="string",F='""'):f==="Password"?(n.tableDisplay&&i.push(I(n.name)),c.push(Ae(n)),x="string",F='""'):f==="ColorPicker"?(n.tableDisplay&&i.push(I(n.name)),c.push(De(n)),x="string",F='""'):f==="Editor"&&(n.tableDisplay&&i.push(I(n.name)),c.push(Le(n)),x="string",F='""'),A===0&&(t+=`id?: string;
`,s+=`id: undefined,
`),t+=`${n.name}: ${x};
`,s+=`${n.name}: ${F},
...
`).forEach(n=>{n.includes("const saveEntity = ")?(d.forEach(({fieldName:A,options:f})=>{g.push(`const ${A}Options = ${JSON.stringify(f,null,2)};
...
`).forEach(n=>{n.includes("const saveEntity = ")?(d.forEach(({fieldName:A,options:f})=>{_.push(`const ${A}Options = ${JSON.stringify(f,null,2)};
...
`),qe=[`<Route path="${e.toLowerCase()}" element={<${e}Page />} />`,`<Route path="${e.toLowerCase()}/create" element={<Create${e}Page />} />`,`<Route path="${e.toLowerCase()}/edit/:id" element={<Edit${e}Page />} />`],Be=[`import ${e}Page from "./screens/${e}/${e}"`,`import Edit${e}Page from "./screens/${e}/Edit${e}"`,`import Create${e}Page from "./screens/${e}/Create${e}"`],N=[];N.push(...Be),Me.forEach(n=>{n.includes("{/* --ROUTES-- */}")&&qe.forEach(A=>{N.push(A);}),N.push(n);}),h.writeFileSync("./src/main.tsx",N.join(`
