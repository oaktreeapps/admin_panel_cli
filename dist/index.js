...
></Column>`,Je=e=>`<Column header="${R(e)}" body={(rowData) => imageBodyTemplate(rowData, "${e}")}></Column>`;var j={tableColumns:"{/*TABLE_COLUMNS*/}",initialState:"/*INITIAL_STATE_FIELDS*/",interface:"/*INTERFACE_FIELDS*/",input:"{/*INPUT_FIELDS*/}",validate:"/*VALIDATE_FIELDS*/"};function ye(e,t){return a(this,null,function*(){let s=W(),r="",i="",l=[],o=[],c=[],m=[],d=["InputSwitch"];t.crudFields.forEach((n,k)=>{let y=n.widget||n.datatype,C="",S="";n.required&&!d.includes(n.widget||n.datatype||"")&&l.push(n.name),y==="InputText"||y==="String"?(n.tableDisplay&&c.push(E(n.name)),o.push(Ae(n)),C="string",S='""'):y==="InputTextarea"?(n.tableDisplay&&c.push(E(n.name)),o.push(_e(n)),C="string",S='""'):y==="InputNumber"||y==="Number"?(n.tableDisplay&&c.push(E(n.name)),o.push(je(n)),C="number",S="0"):y==="Dropdown"?(n.tableDisplay&&c.push(E(n.name)),o.push(Oe(n)),m.push({fieldName:n.name,options:n.options}),C="string",S='""'):y==="RadioButton"?(n.tableDisplay&&c.push(E(n.name)),o.push(ke(n)),m.push({fieldName:n.name,options:n.options}),C="string",S='""'):y==="MultiSelect"?(n.tableDisplay&&c.push(E(n.name)),o.push(He(n)),m.push({fieldName:n.name,options:n.options}),C="string[]",S="[]"):y==="ImageFileUpload"?(n.tableDisplay&&c.push(Je(n.name)),o.push(We(n,t.name.toLowerCase())),C="string",S='""'):y==="InputSwitch"||y==="Boolean"?(n.tableDisplay&&c.push(E(n.name)),o.push(Ue(n)),C="boolean",S="false"):y==="Calendar"?(n.tableDisplay&&c.push(E(n.name)),o.push(Ke(n)),C="string",S='""'):y==="Password"?(n.tableDisplay&&c.push(E(n.name)),o.push(Me(n)),C="string",S='""'):y==="ColorPicker"?(n.tableDisplay&&c.push(E(n.name)),o.push(qe(n)),C="string",S='""'):y==="Editor"&&(n.tableDisplay&&c.push(E(n.name)),o.push(Be(n)),C="string",S='""'),k===0&&(r+=`id?: string;
`,i+=`id: undefined,
`),r+=`${n.name}: ${C};
`,i+=`${n.name}: ${S},
...
`).forEach(n=>{n.includes("const saveEntity = ")?(m.forEach(({fieldName:k,options:y})=>{T.push(`const ${k}Options = ${JSON.stringify(y,null,2)};
...
`).forEach(n=>{n.includes("const saveEntity = ")?(m.forEach(({fieldName:k,options:y})=>{L.push(`const ${k}Options = ${JSON.stringify(y,null,2)};
...
`),ze=[`<Route path="${e.toLowerCase()}" element={<${e}Page />} />`,`<Route path="${e.toLowerCase()}/create" element={<Create${e}Page />} />`,`<Route path="${e.toLowerCase()}/edit/:id" element={<Edit${e}Page />} />`],et=[`import ${e}Page from "./screens/${e}/${e}"`,`import Edit${e}Page from "./screens/${e}/Edit${e}"`,`import Create${e}Page from "./screens/${e}/Create${e}"`],z=[];z.push(...et),Ze.forEach(n=>{n.includes("{/* --ROUTES-- */}")&&ze.forEach(k=>{z.push(k);}),z.push(n);}),X.writeFileSync("./src/main.tsx",z.join(`
