...
`));let O=re.replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(R.input,a.join(`
`)).replace(R.validate,`if (${p.map(o=>`entity.${o}`).join(" && ")}) `),h=[];O.split(`
`).forEach(o=>{o.includes("const saveEntity = async () => {")?(g.forEach(({fieldName:L,options:d})=>{h.push(`const ${L}Options = ${JSON.stringify(d,null,2)};
`);}),h.push(o)):(o.includes(R.initialState)&&h.push(s),h.push(o));}),f.writeFileSync(k,h.join(`
...
`).forEach(o=>{o.includes("const saveEntity = async () => {")?(g.forEach(({fieldName:L,options:d})=>{_.push(`const ${L}Options = ${JSON.stringify(d,null,2)};
`);}),_.push(o)):(o.includes(R.initialState)&&_.push(s),_.push(o));}),f.writeFileSync(te,_.join(`
`));let Oe=f.readFileSync(V),$e=JSON.parse(Oe.toString());$e[0].items.push({label:e,to:`/${e.toLowerCase()}`}),f.writeFileSync(V,JSON.stringify($e,null,2));let Me=se.replace(/XXXXX/g,e).replace(R.interface,t);f.writeFileSync(`./src/types/${e.toLowerCase()}.d.ts`,Me);let qe=f.readFileSync("./src/main.tsx").toString().split(`
...
`;}),r};var u={interface:"/*INTERFACE_FIELDS*/",schema:"/*SCHEMA_FIELDS*/",entity:"/*ENTITY_FIELDS*/",zod:"/*ZOD_FIELDS*/",uniqueFields:"/*UNIQUE_FIELDS*/",checkExistingUpdateEntity:"/*CHECK_EXISTING_UPDATE_ENTITY*/",checkExistingCreateEntity:"/*CHECK_EXISTING_CREATE_ENTITY*/"};function ge(e,n){return c(this,null,function*(){let r=j(),t=n.crudFields.filter(m=>m.unique).map(m=>m.name),s=[],p=[],a=[],i=[];n.crudFields.forEach(({name:m,widget:$,datatype:O,required:h,unique:M})=>{s.push(`${m}: entity.${m},`),$==="InputText"||$==="InputTextarea"||$==="Dropdown"||$==="RadioButton"||$==="ImageFileUpload"||$==="Calendar"||$==="Password"||$==="ColorPicker"||$==="Editor"||O==="String"?(p.push(`${m}${h?"":"?"}: string;`),a.push(`${m}: { type: String, required: ${h}, unique: ${M} },`),i.push(`${m}: z.string()${h?".nonempty()":".optional().nullable()"},`)):$==="InputNumber"||O==="Number"?(p.push(`${m}${h?"":"?"}: number;`),a.push(`${m}: { type: Number, required: ${h}, unique: ${M} },`),i.push(`${m}: z.number()${h?"":".optional().nullable()"},`)):($==="InputSwitch"||O==="Boolean")&&(p.push(`${m}${h?"":"?"}: boolean;`),a.push(`${m}: { type: Boolean, required: ${h}, unique: ${M} },`),i.push(`${m}: z.boolean()${h?"":".optional().nullable()"},`));});let g=`./src/Microservices/${e}`,X=f.readFileSync(`${r}/server/XXXXXController.ts`).toString().replace(/XXXXX/g,e).replace(/xxxxx/g,e.toLowerCase()).replace(u.interface,p.join(`
...
`).filter(P=>g.filter(k=>P.includes(k)).length===0);f.writeFileSync("./src/main.tsx",E.join(`
