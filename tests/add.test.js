...
test.concurrent('new resource has routes, screens & types files', async () => {
    const checks = [
        // webapp
    ...
    // imports checks
    expect(webappRouterFileContent).toContain(`import StudentsPage from "./screens/Students/Students"`)
    expect(webappRouterFileContent).toContain(`import EditStudentsPage from "./screens/Students/EditStudents"`)
    expect(webappRouterFileContent).toContain(`import CreateStudentsPage from "./screens/Students/CreateStudents"`)
    // routes check
    expect(webappRouterFileContent).toContain(`<Route path="students" element={<StudentsPage />} />`)
    expect(webappRouterFileContent).toContain(`<Route path="students/edit/:id" element={<EditStudentsPage />} />`)
    expect(webappRouterFileContent).toContain(`<Route path="students/create" element={<CreateStudentsPage />} />`)

    const serverRouterFileContent = fs.readFileSync(`server/src/Microservices/ApiRouter.ts`).toString()

    // routes check
    expect(serverRouterFileContent).toContain(`ApiRouter.use("/students", StudentsRouter)`)
    // imports check
    expect(serverRouterFileContent).toContain(`import { StudentsRouter } from "./Students/StudentsRouter"`)
}, 50_000);